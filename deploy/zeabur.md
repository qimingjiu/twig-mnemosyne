...
- `litellm`：`LITELLM_MASTER_KEY`、各 provider key（含 `MOONSHOT_API_KEY`；用到 CommandCode/OpenCode/硅基流动中转模型时还需 `COMMANDCODE_API_KEY` / `OPENCODE_API_KEY` / `SILICONFLOW_API_KEY`）、`OLLAMA_API_BASE` 可不设（本地 lane 形态 A 时指向 tailnet）
